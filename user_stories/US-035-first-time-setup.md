...
- [x] A final summary screen lists every completed step and prints "SystemSentinel is running" with a tip for verifying connectivity (e.g. "Send `!status` in your chat channel")
- [x] Re-running `sentinel setup` on an already-configured system is fully safe and idempotent
- [x] A `--check` flag runs all checks and reports status without installing or changing anything
- [x] A `--unattended` flag skips interactive prompts and applies all defaults (for automated provisioning)
