...
## Acceptance Criteria
- [ ] The daemon detects and alerts on the following anomaly patterns:
...
- [ ] Each anomaly type can be individually enabled or disabled in `config.yaml`
- [ ] An alert is sent via chat immediately when an anomaly is detected, with full context (user, IP, time, anomaly type)
- [ ] Detected anomalies are stored in SQLite for historical review
- [ ] The `!anomalies` chat command lists anomalies detected in the last 24 hours
