...
- [x] The chart renderer is pluggable via `charts.renderer` in `config.yaml`; the default renderer sends a unicode text chart as a chat code block (no extra dependencies); an image renderer (PNG attachment) is available as an opt-in extra
