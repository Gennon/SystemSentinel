...
## Acceptance Criteria
- [x] `DigestBuilder` assembles a single structured message from the following sections and sends it at a configurable time (default: 08:00 local time):
  - System uptime
...
- [x] The digest is sent as a structured chat message with a title, timestamp, and one field per section
- [x] If the daemon was offline for part of the 24-hour window, the digest notes the gap
- [x] The digest send time is configurable in `config.yaml`
- [x] Only one digest message is sent per day; it is not duplicated even if multiple chat adapters are active
