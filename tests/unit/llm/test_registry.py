from __future__ import annotations

from importlib.metadata import EntryPoint
import logging
from typing import Any
from unittest.mock import AsyncMock, MagicMock, patch

from system_sentinel.core.context import AppContext
from system_sentinel.llm.base import BaseLLMProvider, LLMRequest, LLMResponse
from system_sentinel.llm.registry import LLMRegistry


def _make_ctx() -> AppContext:
    return AppContext(
        audit=AsyncMock(),
        event_bus=AsyncMock(),
        logger=logging.getLogger("test"),
    )


class _FakeProvider(BaseLLMProvider):
    name = "fake"

    async def complete(self, request: LLMRequest) -> LLMResponse:
        _ = request
        return LLMResponse(text="ok", model_used="fake-model", provider=self.name)

    async def list_models(self) -> list[str]:
        return ["fake-model"]

    async def health_check(self) -> bool:
        return True


def _make_registry(config: dict[str, Any]) -> LLMRegistry:
    return LLMRegistry(config, _make_ctx())


def _make_entry_point(name: str, cls: type) -> MagicMock:
    ep = MagicMock(spec=EntryPoint)
    ep.name = name
    ep.load.return_value = cls
    return ep


def test_discover_loads_enabled_provider() -> None:
    registry = _make_registry({"fake": {"enabled": True}})
    with patch(
        "system_sentinel.llm.registry.entry_points",
        return_value=[_make_entry_point("fake", _FakeProvider)],
    ):
        registry.discover()
    assert "fake" in registry.providers


def test_discover_skips_disabled_provider() -> None:
    registry = _make_registry({"fake": {"enabled": False}})
    with patch(
        "system_sentinel.llm.registry.entry_points",
        return_value=[_make_entry_point("fake", _FakeProvider)],
    ):
        registry.discover()
    assert "fake" not in registry.providers


def test_discover_handles_load_error_gracefully() -> None:
    registry = _make_registry({"bad": {"enabled": True}})
    bad_ep = MagicMock(spec=EntryPoint)
    bad_ep.name = "bad"
    bad_ep.load.side_effect = ImportError("missing dep")
    with patch("system_sentinel.llm.registry.entry_points", return_value=[bad_ep]):
        registry.discover()
    assert "bad" not in registry.providers


def test_providers_property_returns_copy() -> None:
    registry = _make_registry({"fake": {"enabled": True}})
    with patch(
        "system_sentinel.llm.registry.entry_points",
        return_value=[_make_entry_point("fake", _FakeProvider)],
    ):
        registry.discover()
    providers = registry.providers
    providers.clear()
    assert "fake" in registry.providers
