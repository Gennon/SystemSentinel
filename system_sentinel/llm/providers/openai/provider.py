...
class OpenAIProvider(BaseLLMProvider):
    ...
    async def complete(self, request: LLMRequest) -> LLMResponse:
        ...
        if model is None:
            raise LLMUnavailableError("OpenAI provider requires a model in config or request.")
        ...
        if not isinstance(data, dict):
            raise LLMUnavailableError("OpenAI response payload was not a JSON object.")
    ...
    async def list_models(self) -> list[str]:
        ...
        try:
            ...
            raise LLMUnavailableError(f"OpenAI model listing failed: {exc}") from exc

        data = response.json()
        if not isinstance(data, dict):
            raise LLMUnavailableError("OpenAI model listing payload was not a JSON object.")
        rows = data.get("data")
    ...
    def _headers(self) -> dict[str, str]:
        api_key = require_config_string(self.config, "api_key", provider_name="OpenAI")
        return {"Authorization": f"Bearer {api_key}"}

    def _endpoint(self, path: str) -> str:
        base = as_string(self.config.get("endpoint")) or "https://api.openai.com"
...
def _extract_openai_text(data: dict[str, Any]) -> str:
    ...
    if not isinstance(choices, list) or not choices:
        raise LLMUnavailableError("OpenAI response did not include any choices.")
    first = choices[0]
    if not isinstance(first, dict):
        raise LLMUnavailableError("OpenAI response choice was invalid.")
    message = first.get("message")
    if not isinstance(message, dict):
        raise LLMUnavailableError("OpenAI response choice missing message payload.")
    content = message.get("content")
    if not isinstance(content, str) or not content.strip():
        raise LLMUnavailableError("OpenAI response content was empty.")
