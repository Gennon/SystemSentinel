...
### System Maintenance
- As a user I want a single setup command that launches a friendly wizard so I can go from a fresh Linux machine to a running daemon without prior knowledge.
- As a user I want all mandatory dependencies installed automatically during setup so the daemon works out of the box.
